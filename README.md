...
### 🪟 Windows 用户：双击 run.bat 一键操作 / Windows Users: Double-click run.bat

项目自带 `run.bat` 批处理文件，**双击直接打开图形界面**，无需任何命令。GUI 提供完整的交互体验：

| 特性 / Feature | 说明 / Description |
|----------------|-------------------|
| 🎯 **拖拽文件夹 / Drag & Drop** | 将文件夹从文件管理器拖到 `run.bat` 图标上 → 自动在 GUI 中加载；也可在 GUI 中点击浏览或拖拽 |
| 🖥️ **图形界面操作 / GUI** | 点击按钮选择文件夹、切换模式（单文件/门户）、设置字数限制、一键生成 |
| 📋 **实时文件列表 / File Preview** | 扫描后立即显示所有文件及其大小、状态（支持/跳过），一目了然 |
| 🌐 **中英双语 / Bilingual** | 所有界面文字同时显示中文和英文 |
| 🎛️ **命令行菜单备用 / CLI Fallback** | 如果 GUI 不可用，关闭后自动进入命令行菜单 |

**使用方法：**

| 操作 / Action | 结果 / Result |
|---------------|---------------|
| **双击 `run.bat`** | 直接打开 GUI 图形界面 |
| **拖文件夹到 `run.bat` 图标** | 打开 GUI 并自动加载该文件夹 |
| **关闭 GUI 后** | 可选择重新启动或进入命令行高级模式 |
...
| 🗂️ **文件夹结构树** | 门户首页左侧显示完整文件夹树，点击跳转 |
| 🔄 **语言同步持久化** | GUI 语言偏好自动保存，下次启动自动恢复 |
| 🧹 **Clear 按钮** | 一键清空当前文件夹加载状态 |
| 📂 **拖拽支持** | 支持文件夹拖入 GUI（需 tkinterdnd2） |
| 🔗 **跳过文件独立显示** | 不支持的文档仅显示在文件树中，不生成卡片页面 |
