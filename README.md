...
> **Turn folders into AI-readable knowledge portals — zero API, zero model, zero server.**

---

## 中文 / Chinese

### DocPortal — 为网页 AI 打造的一键式本地文档门户
...
「RAG（检索增强生成）」通常需要嵌入模型、向量数据库和语言模型协同工作——这意味着 GPU、联网、复杂的部署和持续维护。DocPortal 完全放弃了这条重路线，转而把文档直接做成 AI 可以直接读取的网页：每一页都自动控制长度，确保 Edge Copilot、ChatGPT、Claude 等能完整消费，而无需任何插件或文件上传。
...
> **Note**: The name "DocPortal" was chosen to reflect its purpose — turning document folders into AI-readable knowledge portals. The earlier codename "FolderRAG" was used during initial development; "RAG" in that context referred to organizing folder contents into structured knowledge packages for AI retrieval, not a full Retrieval-Augmented Generation pipeline.
...
- **🔗 RAG 预处理 / RAG preprocessing**：作为 RAG 管道的文档加载和清洗步骤 / Document loading and cleaning for RAG pipelines
