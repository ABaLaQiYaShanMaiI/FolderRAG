#!/usr/bin/env python3
"""
FolderRAG Desktop — 文件夹知识导出图形界面工具 | Bilingual GUI (中文/English)
...
def build_html_from_files(
    ...
    html = (
        ...
        f"{articles_html}\n"
        "  <div class=\"footer\">\n"
        f"    <p>{_('由 FolderRAG Desktop 生成', 'Generated by FolderRAG Desktop')} | {_('共', 'Total')} {file_count} {_('个文件', 'files')}，{total_chars:,} {_('字符', 'chars')}"
        f" | {now}</p>\n"
        "  </div>\n"
        "</body>\n"
        "</html>"
...
class FolderRAGUI:
    """Main GUI Application for FolderRAG — bilingual (中文/English)."""
    ...
    def __init__(self, root):
        self.root = root
        self.root.title(_("FolderRAG Desktop 🚀", "FolderRAG Desktop 🚀"))
    ...
    def build_ui(self):
        ...
        ttk.Label(header_frame, text=_("FolderRAG Desktop", "FolderRAG Desktop"), style='Title.TLabel').pack(side=tk.LEFT)
